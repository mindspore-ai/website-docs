...
$(document).ready(function () {
        ...
        res.forEach(function (item) {
          item.children && (item.subType === 2) && item.children.forEach(function (item) {
              ...
              item.list.forEach(function (item) {
                  if((pathname.indexOf(item.containUrl) === 0) && !subNav.list.length) {
    ...
    if (VERSION !== '0.1.0-alpha' && VERSION !== '0.2.0-alpha' && VERSION !== '1.5' && VERSION !== '1.6'&& VERSION !== 'master'&& VERSION !== '1.7'&& VERSION !== '1.8') {
        ...
        if(VERSION !== 'master' &&VERSION !== 'r1.5'&&VERSION !== 'r1.6'&& VERSION !== 'r1.7'&& VERSION !== 'r1.8'&& VERSION !== 'r1.9'&& VERSION !== 'r1.10') {
            ...
            let $elm = $('.wy-menu-vertical>ul>.toctree-l1>a,.toctree-wrapper>ul>.toctree-l1>a').filter(function(index,element){
                if (element.innerText == 'Operator List') {
                    return element
                }
            })
            let tarElm =
            '<li class="toctree-l1"><a class="reference internal" href="#operator">Operator Mapping</a>' +
            '<ul><li class="toctree-l2"><a class="reference internal" href="#operator_api">API Mapping</a></li>' +
            "</ul>" +
            "</li>";
            //插入算子支持的后面
        ...
        function replaceContent(params) {
            ...
            if(host.pathname.includes('/docs/migration_guide/en/master/api_mapping/pytorch_api_mapping.html')) {
            ...
            if (curHash === "#operator_api" || curHash === "#operator_param") {
                ...
                if(VERSION !== 'master'&&VERSION !== 'r1.5'&&VERSION !== 'r1.6'&&VERSION !== 'r1.7'&&VERSION !== 'r1.8'&& VERSION !== 'r1.9'&& VERSION !== 'r1.10') {
                    //面包屑的内容
                    $('.wy-breadcrumbs>li:nth-of-type(2)')[0].innerHTML = "Operator Mapping&nbsp;>&nbsp;";
                    if ( $('.wy-breadcrumbs>li:nth-of-type(3)')[0].className !== 'wy-breadcrumbs-aside') {
                        $('.wy-breadcrumbs>li:nth-of-type(3)').empty()
                        $('.wy-breadcrumbs>li:nth-of-type(3)').append(curHash == "#operator_api"?"API Mapping":"参数映射")
                        // $('.wy-breadcrumbs>li:nth-of-type(3)')[0].innerHTML = curHash == "#operator_api"?"API Mapping":"参数映射";
                    } else {
                        $('.wy-breadcrumbs>li:nth-of-type(2)').after(('<li>'+(curHash == "#operator_api"?"API Mapping":"参数映射")+'</li>'))
                    }
                }
                //定义api映射内容元素
                let inputElm = `
                ...
                function getApiDetailAndReplaceContent(op) {
                    $.get(mapingApiUrl, { framework: "PyTorch", op: op, version:pyVersion,msVersion:msVersion}, function(data,status) {
                        ...
                        $('#mindsporeFrame').append($(mindsporeHtml))
                         ...
                         let diffLink = '';
                         let diffHtml = '';
                         let diffAPILink = 'https://www.mindspore.cn/docs/migration_guide/en/master/api_mapping/pytorch_api_mapping.html';
                         if(diffstring.length > 0) {
                            if(diffstring.indexOf('[diff]') !== -1) {
                                diffstringTitle = 'Difference Comparison' ;
                                ...
                                diffHtml = `
                                     <div class="diff-content">
                                        ...
                                        <div class="diff-content-enclosure">
                                            ...
                                            <a target="_blank" href=${diffAPILink} > API Mapping List </a>
                                            provided in the community.(Operators are continuously updated...)
                                        </div>
                                     </div>
                                `
                            } else {
                                if(diffstring.indexOf('diff') === 0) {
                                    ...
                                    diffstringTitle = 'Consistent functions';
                                }
                                diffHtml = `
                                   <div class="diff-content">
                                        ${diffstringTitle}
            ...
            if(curHash === "#operator_api" ){
                ...
                VERSION=='r1.1'
                 ...
                 ${
                    `<p>For details about the mapping between PyTorch APIs and MindSpore APIs, see <a href="https://gitee.com/mindspore/docs/blob/${VERSION}/resource/${isBigthen12?"api_mapping/":""}${isBigthen12?"api_mapping_en.md":"api_mapping.md"}">API Mapping</a>.</p>`
                 }
                 </div>`
    ...
    if(pathname.indexOf('/doc/api_python/') !== -1) {
        ...
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', 'Search in Migration Guide');
        ...
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', 'Search in Docs');
    }
    ...
    if (pathname == "/en/master/index.html") {
        let h1List = $('h1');
    ...
    for(let i=0;i<doms.length;i++){
        if(doms[i].scrollWidth>doms[i].clientWidth||doms[i].scrollWidth>doms[i].getBoundingClientRect().width){
