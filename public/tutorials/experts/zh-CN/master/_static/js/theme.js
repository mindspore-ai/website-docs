...
$(document).ready(function () {
    ...
    // 教程--初级目录结构不一致
    if((pathname.split('/')[1] === 'tutorial'|| path.split('/')[0] === 'tutorials') && 
    ...
    if(host.pathname.indexOf('/lite/') == -1){
        $('.wy-menu-vertical').before('<div class="docsHome"><a  href="#" class="welcome">欢迎查看MindSpore教程</a></div>')
    }else{
        $('.wy-menu-vertical').before('<div class="docsHome"><a  href="#" class="welcome">欢迎查看MindSpore Lite教程</a></div>')
    } 
    $('.welcome')[0].attributes[0].nodeValue = $('.icon-home')[0].attributes[0].nodeValue;
    $('.wy-breadcrumbs>li:first-of-type')[0].innerText = '教程 (' + VerText + ')';
    ...
    if(pathname.indexOf('/tutorial/lite/') !== -1) {
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', '"手机及loT" 内搜索');
    } else if(pathname.indexOf('/tutorial/training/') !== -1) {
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', '"训练" 内搜索');
    } else if(pathname.indexOf('/tutorial/inference/') !== -1) {
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', '"推理" 内搜索');
    } else if(pathname.indexOf('/tutorial/') !== -1) {
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', '"快速入门" 内搜索');
    } else if(pathname.indexOf('/tutorials/') !== -1){
        $('.wy-grid-for-nav #rtd-search-form input').eq(0).attr('placeholder', '"教程" 内搜索');
    }
    ...
    if (pathname == "/tutorial/zh-CN/master/index.html" || pathname == "/lite/tutorial/zh-CN/master/index.html") {
        let h1List = $('h1');
    ...
    // 点击设置开发环境跳转（需要改）
    if (pathname == '/tutorial/zh-CN/master/advanced_use/community/CONTRIBUTING_CODE.html' || pathname == '/lite/tutorial/zh-CN/master/advanced_use/community/CONTRIBUTING_CODE.html') {
        let linkInstall = $('a[href="../../install/install"]');
        linkInstall.attr('target', "_blank");
        let h2List = $('.section>h2');
        let pList = $('.section>p>a');
        for (let i = 0; i < pList.length; i++) {
            if (pList[i].innerText == 'CLA') {
                pList[i].target = '';
                pList[i].href = "/download?fileName=ContributorLicenseAgreement.md";
            }
        }
        for (let i = 0; i < h2List.length; i++) {
            if (h2List[i].innerText == "编码规范") {
                h2List[i].id = "编码规范";
                let aLinkTarget = $('#' + '' + h2List[i].parentNode.id + '>p>a');
                aLinkTarget.attr('target', "_blank")
            } else if (h2List[i].innerText == "单元测试") {
                let aLinkTarget = $('#' + '' + h2List[i].parentNode.id + '>p>a');
                aLinkTarget.attr('target', "_blank")
            }
        }
        if (pathname.indexOf('/zh-CN/') == -1) {
            linkInstall[0].href = '/install/en';
        } else {
            linkInstall[0].href = '/install';
        }
    }
    // 解决目录滚动问题
