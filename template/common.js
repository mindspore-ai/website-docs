...
${
  ...
  `;
        ...
        if (pathname.startsWith('/tutorials/') && item.id === 'learning') {
          item.active = 1;
        }
        let navLabel = '';
        if (href) {
                      ...
                      .map((sub, index) => {
                        let href = sub.href
                          ? isEn
                            ...
                            : sub.href.zh
                          : '';
                        const name = isEn ? sub.label.en : sub.label.zh;
                        if (sub.id === 'tutorials') {
                          href = href.replace(
                            'master',
                            msHeader.headerPathVersion(sub.id)
...
`;
      ...
      h5HeaderMethods: function () {
        ...
        $('.rating-label').on('click', function () {
          body.prepend(msHeader.dialogLayer(msFotter.fontmatter.helpforyou));
          $('#mask').show();
...
<li>
  ...
  .join('')}
      ...
      // PC footer
      pcFootHTML: function () {
        const { copyRight, beianLink, beian, menu } = msFotter.fontmatter;
        return `
<div id="footer">
          ...
          if (h3.length > 0) {
            ...
            for (let i = 0; i < h3.length; i++) {
              if (h3[i].parentNode.querySelectorAll('h4').length > 0) {
                ...
                navLi3 += sideNodeList(h3[i], true, false, navLi4, 'navList4');
              } else {
                navLi3 += sideNodeList(h3[i]);
              }
            }
            navLi2 = sideNodeList(h2List[i], true, false, '', 'navList3');
          } else {
            navLi3 = '';
            navLi2 = '';
            if (
              ...
              for (let j = 0; j < navLi3Array.length; j++) {
                if (
                  ...
                  for (let k = 0; k < navLi4Array.length; k++) {
                    navLi4 += sideNodeList(navLi4Array[k], false, true);
                  }
                  navLi3 += sideNodeList(
                    navLi3Array[j],
                    true,
...
<ul class="navList3"></ul>
      ...
      function navContentAnchor() {
        for (let i = 0; i < navListLink.length; i++) {
          ...
          let newAnchorId = anchorId.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    ...
    const componentInfo = {
      versionDropdownList: function () {
        ...
        const matchingItem = msVersionData.find(
          (item) =>
            pathname.startsWith(`/${item.name}/`) && item.state !== 'old'
        );
        if (matchingItem) {
